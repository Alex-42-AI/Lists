class SortedList:
    ...
    def pop(self, index: int = -1): return self.__value.pop(index)
    def insert(self, el):
        low, high = 0, len(self)
        while low < high:
            mid = (low + high) // 2
            if self.__f(el) == self.__f(self[mid]):
                ...
                return
            if self.__f(el) < self.__f(self[mid]): high = mid
            else:
                if low == mid + 1: break
                low = mid + 1
    ...
    def copy(self):
        ...
        for el in self: res.insert(el)
        return res
    def merge(self, other):
        if isinstance(other, SortedList):
            if any([self.__f(el) != other.__f(el) for el in self.__value + other.__value]): raise ValueError("Sorting functions of both lists are different!")
            res = self.copy()
            for el in other: res.insert(el)
            return res
    ...
    def __contains__(self, item):
        ...
        while low < high:
            ...
            if self.__f(item) == self.__f(self[mid]):
                if item == self[mid]: return True
                i, j = mid - 1, mid + 1
                while True:
                    if i >= 0 and self.__f(self[i]) == self.__f(item):
                        if item == self[i]: return True
                        i -= 1
                        continue
                    if j < len(self) and self.__f(self[j]) == self.__f(item):
                        if item == self[j]: return True
                        j += 1
                        continue
                    break
                return False
            if self.__f(item) < self.__f(self[mid]): high = mid
            else:
                if low == mid: return False
                low = mid
    ...
    def __setitem__(self, i: int, value): self.remove(self[i]), self.insert(value)
...
def original(data: [int], starter: int = 0, jump: int = 1):
    ...
    res = [0] * len(data)
    for index in range(len(sorted_res)): res[sorted_res[index]] = index * jump + starter
    return res
...
def heapify(ll: [int], l: int, h: int, i: int, f=max):
    ...
    res = i
    if l + left <= h and ll[l + i - 1] != f(ll[l + left - 1], ll[l + i - 1]): res = left
    if l + right <= h and ll[l + res - 1] != f(ll[l + right - 1], ll[l + res - 1]): res = right
    if res != i:
        ll[l + i - 1], ll[l + res - 1] = ll[l + res - 1], ll[l + i - 1]
        heapify(ll, l, h, res, f)
def build_heap(ll: [int], h: int = 0):
    if not h: h = len(ll)
    for i in range(h // 2, 2, -1): heapify(ll, 2, h, i)
def heapsort(ll: [int]):
    build_heap(ll)
    h = len(ll)
